...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask,d_model,n_layers,n_heads,d_k,d_v,d_ff):
        ...
        ## 下面这个用到了attn_mask，把被mask的地方置为无限小，softmax之后基本就是0，对q的单词不起作用
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask,d_model,n_layers,n_heads,d_k,d_v,d_ff):
        ...
        ##下面这个就是先映射，后分头；q和k分头之后维度是一致的，所以一看这里都是dk
        ...
        ##然后计算 ScaledDotProductAttention 这个函数.
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        # print((self.pe[:x.size(0), :]).is_cuda)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask,d_model,n_layers,n_heads,d_k,d_v,d_ff):
        ##自注意力层，输入是enc_inputs，形状是[batch_size x seq_len_q x d_model] 最初始的QKV矩阵是等同于这个输入的，enc_self_attn函数 6.
...
class Encoder(nn.Module):
    def __init__(self,d_model,n_layers,n_heads,d_k,d_v,d_ff):
        ...
        # self.src_emb = nn.Embedding(src_vocab_size, d_model)  ## 定义生成一个矩阵，大小是 src_vocab_size * d_model
        self.pos_emb = PositionalEncoding(d_model) ## 位置编码情况，这里是固定的正余弦函数，也可以使用类似词向量的nn.Embedding获得一个可以更新学习的位置编码
        self.layers = nn.ModuleList([EncoderLayer(d_model,n_layers,n_heads,d_k,d_v,d_ff) for _ in range(n_layers)]) ## 使用ModuleList对多个encoder进行堆叠，因为后续的encoder并没有使用词向量和位置编码，所以抽离出来；
    ...
    def forward(self, enc_inputs,word_seq_tensor,d_model,n_layers,n_heads,d_k,d_v,d_ff):
        ...
        tensor_list = [] #存放transformer每一层的输出

        for layer in self.layers:
            ## EncoderLayer
            enc_outputs, enc_self_attn = layer(enc_outputs, enc_self_attn_mask,d_model,n_layers,n_heads,d_k,d_v,d_ff)
            enc_self_attns.append(enc_self_attn)
            tensor_list.append(enc_outputs)
        # np.save('layer.npy', tensor_list)
        # loaded_array = np.load('layer.npy', allow_pickle=True)
        # print(loaded_array)
        return enc_outputs, enc_self_attns,tensor_list
...
#         ## enc_outputs就是主要的输出，enc_self_attns这里是QK转置相乘之后softmax之后的矩阵值，代表的是每个单词和其他单词相关性；
#         enc_outputs, enc_self_attns,tensor_list = self.encoder(enc_inputs)
#         # print(enc_outputs)
...
class Transformer(nn.Module):
    ...
    def __init__(self,d_model):
        ...
        self.d_k = self.d_v = 56  # dimension of K(=Q), V
    ...
    def forward(self,inputs,word_seq_tensor):
        enc_outputs, enc_self_attns, tensor_list = self.encoder(inputs,word_seq_tensor,self.d_model,self.n_layers,self.n_heads,self.d_k,self.d_v,self.d_ff)
        # print(enc_outputs.size())
        return enc_outputs,tensor_list
