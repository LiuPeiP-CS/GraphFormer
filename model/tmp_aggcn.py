...
class DCGCN(nn.Module):
    def __init__(self, opt, embeddings):
        ...
        self.transformer = Transformer(330)
        self.transformer2 = Transformer(600)
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                self.layers.append(GraphConvLayer(opt, self.mem_dim, 4))
                #self.layers.append(GraphConvLayer(opt, self.mem_dim, 5))
            else:
                #self.layers.append(MultiGraphConvLayer(opt, self.mem_dim, 2, self.heads))
                self.layers.append(MultiGraphConvLayer(opt, self.mem_dim, 4, self.heads))
    ...
    def forward(self, adj, inputs):
        ...
        # for gf_i for range
        # transformer
        embs, _ = self.transformer(embs,words)

        gcn_inputs = self.input_W_R(embs)

        layer_list = [] #gcn每一层的输出
        outputs = gcn_inputs
        ...
        #########第二层
        layer_tensor = torch.cat(layer_list, dim=2)

        # transformer
        embs, _ = self.transformer2(layer_tensor, words)
        gcn_inputs = self.input_W_R2(layer_tensor)
