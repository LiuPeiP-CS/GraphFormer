...
import sys
sys.path.append("..")
from data.Finetuning_BertCRF.common_modules.CRF_classes import CRF
import torch.nn.functional as F
from data.Finetuning_BertCRF.common_modules.utils import decode_tag
...
class BERT_CRF_NER(nn.Module):
    ...
    def get_bert_emission(self, input_ids, input_mask, seg_ids, first_label_mask):
        token_features = self.get_bert_features(input_ids, seg_ids, input_mask)
        wv2features = self.word_embedding(token_features, input_mask, first_label_mask)
        return wv2features  # (batch_size, token_sent_len, bert_token_dim)
