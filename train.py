...
from data.write_loss import write_loss
...
parser.add_argument('--vocab_dir', type=str, default='dataset/bert_vocab_pubmed', choices=['dataset/glove_vocab_pubmed', 'dataset/bert_vocab_pubmed'])
parser.add_argument('--emb_dim', type=int, default=768, choices=[300, 768], help='Word embedding dimension.')
parser.add_argument('--pos_dim', type=int, default=30, help='POS embedding dimension.')
parser.add_argument('--rel_dim', type=int, default=30, help='Dependency relation embedding dimension.')

parser.add_argument('--hidden_dim', type=int, default=300, help='GCN hidden dimension.')
...
parser.add_argument('--topn', type=int, default=-1, help='Only finetune top N word embeddings.')
...
parser.add_argument('--block_num', type=int, default=1, help='Number of graphformer blocks.')
parser.add_argument('--module_sequence_selector', type=int, default=0, choices=[0, 1], help='Order used to select GCN modules and Transformer modules.')
parser.add_argument('--aggregator', type=int, default=2, choices=[0, 1, 2], help='Method for selecting aggregation.')
parser.add_argument('--context_emb', type=str, default="bert", choices=["none", "bert"], help="contextual word embedding")
...
vocab = Vocab(vocab_file, opt, load=True)
...
# balanced.balanced_train_file()
print("Loading data from {} with batch size {}...".format(opt['data_dir'], opt['batch_size']))
data = read_file(opt['data_dir'] + '/train.json', vocab, opt, True)
# dev_data = read_file(opt['data_dir'] + '/dev.json', vocab, opt, True)
# data = read_file(opt['data_dir'] + '/pre_train.json', vocab, opt, True)
# train_data = data
dev_data = data[:200]
train_data = data[200:]

dev_batch = DataLoader(dev_data, opt['batch_size'], opt, evaluation=True)
train_batch = DataLoader(train_data, opt['batch_size'], opt, evaluation=False)
...
write_loss(train_loss_list, dev_loss_list, loss_list)
